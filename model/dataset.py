...
class RandlanetDataset(data.Dataset):
    ...
    def __init__(self, pc_path_list, **kwargs):
		...
		# 1. Height (Z normalized)
            z_norm = (xyz[:, 2] - np.min(xyz[:, 2])) / (np.max(xyz[:, 2]) - np.min(xyz[:, 2]) + 1e-6)

		# 2. Local density (simple approximation using kNN)

            nbrs = NearestNeighbors(n_neighbors=10).fit(xyz)
            distances, _ = nbrs.kneighbors(xyz)
            density = 1.0 / (np.mean(distances, axis=1) + 1e-6)

		# 3. RGB variance (ground = uniform)
            rgb_var = np.var(rgb, axis=1)

		# 4. Approx normal Z (VERY IMPORTANT)
		# quick approximation using covariance
            def compute_normal_z(points):
                normals = []
                for i in range(points.shape[0]):
                    idx = np.random.choice(points.shape[0], 10)
                    neigh = points[idx]
                    cov = np.cov(neigh.T)
                    eigvals, eigvecs = np.linalg.eig(cov)
                    normal = eigvecs[:, np.argmin(eigvals)]
                    normals.append(abs(normal[2]))  # z component
                return np.array(normals)

            normal_z = compute_normal_z(xyz)

		# -------- STACK FEATURES --------
            physics_features = np.column_stack((z_norm, density, rgb_var, normal_z))
            
            pc = np.hstack((xyz, rgb, physics_features, labels.reshape(-1,1)))
            self.full_pc[pc_id] = pc
            ...
            self.labels[pc_id] = pc[:, -1].astype(np.int32)
    ...
    def __getitem__(self, _tuple):
        ...
        queried_pc = points[query_idx]

        queried_pc_xyz = queried_pc[:, :3]
        queried_pc_xyz = queried_pc_xyz - pick_point[:, 0:3]

        queried_pc_colors = queried_pc[:, 3:6]
        queried_pc_physics = queried_pc[:, 6:-1]
        queried_pc_labels = queried_pc[:, -1]
    ...
    def build_input(self, xyz, rgb, physics, labels, query_idx, pc_id):
        # Extract full pc from kd-tree


        # Apply augmentation only on xyz
        aug_xyz = self.augment_xyz(xyz)

        # Combine all features
        physics = (physics - physics.mean(axis=0)) / (physics.std(axis=0) + 1e-6)
        features = np.concatenate([aug_xyz, rgb, physics], axis=-1)
