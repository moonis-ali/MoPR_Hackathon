...
def train_epoch(device, loss_function, model, optimizer,
	...
	# ---------- CLASS WEIGHT ----------
        ...
        ce_loss = ce_loss * one_hot
        ce_loss = ce_loss.mean()

	# ---------- PHYSICS FEATURES ----------
        features = inputs['features']   # (B, N, F)
	...
	# ---------- GROUND MASK ----------
	# IMPORTANT: change label if ground is not 0
        ground_mask = (labels == 1).float()

	# ---------- PHYSICS LOSSES ----------
	# 1. Ground should be flat → normal_z ~ 1
        L_normal = torch.mean(ground_mask * (1 - normal_z)**2)

	# 2. Ground should be dense → penalize low density
        L_density = torch.mean(ground_mask * (1.0 / (density + 1e-6)))

	# 3. Ground should be uniform color → low variance
        L_rgb = torch.mean(ground_mask * rgb_var)
	...
	# ---------- FINAL LOSS ----------
        ...
        loss = ce_loss + lambda_n * L_normal + lambda_d * L_density + lambda_r * L_rgb
...
def validation(device, model, test_loader, n_layers, n_classes, scheduler):
    ...
    with torch.no_grad():
        for cnt, input_list in enumerate(test_logger):
            ...
            conf_matrix = confusion_matrix(labels, pred, labels=np.arange(0, n_classes, 1))
